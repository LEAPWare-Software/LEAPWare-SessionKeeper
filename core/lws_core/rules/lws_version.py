"""lws_version: the walking-skeleton rule -- a safe no-op.

Fires on every event, unconditionally, and reports this build's
`lws_core.__version__` as a WARN-shaped finding. It never denies: even if
...
the shipped default. That is deliberate: LWS sits next to LWH (which
...
docs/rules/lws-version.md for the policy-author-facing description.
"""
...
# Not `from .. import __version__`: lws_core/__init__.py imports engine,
# which imports rules, which imports this module -- importing the package
# itself here would be a circular import. lws_core/__init__.py re-exports
# the canonical `__version__`; that module is this string's one source of
# truth, kept in sync by tests/core/test_lws_version.py.
_VERSION = "0.1.0"
...
def evaluate(event: Event, config: RuleConfig):
    ...
    return Finding(
        rule_id=rule_id,
        mode=RuleMode.WARN,
        reason=f"lws {_VERSION}: reporting only, no policy enforced yet",
    )
