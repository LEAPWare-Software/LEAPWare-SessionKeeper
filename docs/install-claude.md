# Installing sessionkeeper on Claude Code
...
1. In Claude Code, add this repository as a marketplace source pointing at
   ...
   `plugins/claude/lws` directly as a local plugin path, per Claude Code's
   own plugin-development docs.
2. Before installing, run `python scripts/lws_build.py` from the repo root so
   `plugins/claude/lws/vendor/` contains a current copy of `lws_core` and
   `adapters/claude` — the plugin cannot import from outside its own
   directory once installed.
3. Enable the `sessionkeeper` plugin.

## What it registers

One hook, in `plugins/claude/lws/hooks/hooks.json`:
...
```json
{
  ...
  "hooks": [
    {
      "type": "command",
      "command": "python3 \"${CLAUDE_PLUGIN_ROOT}/bin/lws_hook.py\" || python \"${CLAUDE_PLUGIN_ROOT}/bin/lws_hook.py\""
    }
  ]
}
```
...
`plugins/claude/lws/bin/lws_hook.py` reads the `PreToolUse` JSON from
...
see `docs/rules/budget-line.md` and `lws-report`'s `SKILL.md` for how to
read it.
