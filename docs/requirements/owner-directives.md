...
2026-09-17 directives for LWR, plus this setup session's own elaboration
of them; see the memory record `lwr-directives-2026-09-17.md` for the
owner's original, unelaborated wording.

1. Code name LWR; every command, skill, and user-facing entrypoint starts
   with `lwr`. Own public repo
   `github.com/LEAPWare-Software/LEAPWare-Runway`, set up exactly like
   LWH: identity `LEAPWare <leapware@outlook.com>`, rulesets, merge
   queue, squash, hosted CI, handoff protocol, proof records.
2. LWR is a rate-limit runway plugin: it watches a CLI session's own
...
6. Status line: shows ONE SHORT TAG for the session's runway state
   (proposed: `LWR ok`, `LWR WIND 80%`, `LWR LAND 90%`, `LWR HOFF 95%`,
   `LWR RET` for retired — final label text needs owner approval before
   freeze) and must never overwrite an existing status line segment.
   Usage readings (context %, 5h %, 7d %) are shared with LWH: LWH owns
   the usage *display* when it is present; LWR reads the same
...
10. Open source, public: `github.com/LEAPWare-Software/LEAPWare-Runway`,
...
12. Repository rulesets (no bypass), merge queue + auto-merge, squash
    merges only; one GitHub App per CLI (`lwr-claude`, `lwr-codex`).
...
14. Lean: LW-Watchtower became bloated; lwr must stay small, with a size
...
15. Automatic restart after a rate-limit reset is IN SCOPE: build it and
    ...
    restart).
16. Out of scope for lwr: usage *display* in the status line when LWH is
...
17. Prove without doubt: a numbered testable requirements list; a test
    ...
    rehearsal — proof record only); a one-day comparison of LWR's
