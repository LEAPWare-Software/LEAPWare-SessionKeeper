...
`core/lwr_core/rules/lwr_version.py`. Default mode: `warn` (see
...
configures it to `deny`. CTO decision, 2026-09-17: LWR must not ship
...
A `Finding` whose `reason` states this build's `lwr_core.__version__`,
e.g. `"lwr 0.1.0: reporting only, no policy enforced yet"`. The `mode` on
...
on the render — works end-to-end before LWR's real runway rules
(wind-down / landing / handoff-only / retire) are designed and built on
top of the same scaffolding. See `tests/core/test_lwr_version.py`,
