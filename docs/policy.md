...
A runway policy is one JSON file matching `core/policy/schema.json`:
...
- `"rules"` is an object keyed by `rule_id` — see
  `core/lwr_core/rules/__init__.py`'s `RULES` list for every rule id a
  build of runway knows about.
...
finding (see `core/lwr_core/engine.py`); `warn` findings from rules that
...
- `lwr_core.config.load_policy_dict(None)` — or any non-dict input —
...
- A single rule entry that isn't an object, or whose `"mode"` isn't a
  ...
  other rule in the same file (see `lwr_core/config.py::load_policy_dict`).
- `plugins/claude/lwr/bin/lwr_hook.py` turns "file doesn't exist" and
...
- Claude Code plugin: `LWR_POLICY_PATH` env var if set, otherwise the
  vendored `core/policy/default.json` bundled at
  `plugins/claude/lwr/vendor/policy/default.json`.
- Codex plugin: same convention, read by the `lwr-config` /
  `lwr-report` skills rather than a hook (see `docs/install-codex.md`).
