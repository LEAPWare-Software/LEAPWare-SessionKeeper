...
Usage:
    python scripts/lwr_handoff.py --check                      # validate HANDOFF.md, exit 1 on failure
    python scripts/lwr_handoff.py --write [--cli NAME] [--session ID]
...
BEGIN_MARKER = "<!-- lwr-handoff:begin -->"
END_MARKER = "<!-- lwr-handoff:end -->"
...
def _proof_state_lines() -> list[str]:
    ...
    Reuses scripts/lwr_check_proof.py's own record validator so this never
    drifts from what the `lwr-proof` CI job itself enforces. An empty
    proof/ directory (the common case today -- see proof/README.md) is not
    an error; it just means the list is "(none yet)".
    """
    import lwr_check_proof  # local import: only --write needs this, --check must not
    ...
    for path in records:
        try:
            ...
            continue
        errors = lwr_check_proof._validate_record(path, data)
