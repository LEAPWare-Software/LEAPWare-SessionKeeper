...
def _check_prs_live(section_text: str, exclude_pr: int | None = None) -> list[str]:
    """Re-derive the live set of open PR numbers and compare it against
    the numbers named in HANDOFF.md's "Open PRs:" section.

    `exclude_pr`, when given, is dropped from *both* sides before
    comparing -- the same structural reasoning as MAX_SHA_LAG for the SHA
    check: the block is generated (and its commit written) *before* that
    commit's own pull request exists, so it can neither have named it
    (generated before the PR was opened) nor be faulted for naming it
    (generated by a later --write run in the same PR, after gh already
    sees it as open) -- either is fine, only a mismatch on any *other*
    number is a real defect. In CI this is the PR number the workflow is
    running for (--pr-number / $GITHUB_EVENT pull_request.number).
    """
    ...
    if exclude_pr is not None:
        live.discard(exclude_pr)
        committed.discard(exclude_pr)
...
def cmd_check_live(pr_number: int | None = None) -> int:
    ...
    `pr_number`, when given (CI passes the PR this run is for), is excused
    from the open-PR comparison -- see _check_prs_live()'s docstring for
    why: the same structural reasoning as MAX_SHA_LAG, just for PR numbers
    instead of commits.
    """
    base_rc = cmd_check()
    if base_rc != 0:
    ...
    try:
        ...
        errors.extend(_check_prs_live(pr_section, exclude_pr=pr_number))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="--check-live only: this run's own PR number, excused from the open-PR comparison "
        "(a block committed before its own PR exists cannot have named it)",
    )
    ...
    if args.check:
        return cmd_check()
    if args.check_live:
        return cmd_check_live(pr_number=args.pr_number)
