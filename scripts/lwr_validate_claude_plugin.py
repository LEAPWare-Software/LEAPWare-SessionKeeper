#!/usr/bin/env python3
"""Validate plugins/claude/lwr against the shape Claude Code plugins require.
...
Checks (stdlib only):
  ...
  - vendor/lwr_core and vendor/adapters/claude exist (i.e. scripts/lwr_build.py
...
PLUGIN_DIR = REPO_ROOT / "plugins" / "claude" / "lwr"
...
def validate() -> list[str]:
    ...
    if not (PLUGIN_DIR / "bin" / "lwr_hook.py").is_file():
        errors.append("missing plugins/claude/lwr/bin/lwr_hook.py")
    ...
    if isinstance(marketplace, dict):
        ...
        if "./plugins/claude/lwr" not in sources:
            errors.append(
