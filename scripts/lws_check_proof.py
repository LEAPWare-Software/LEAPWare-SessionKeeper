#!/usr/bin/env python3
"""CI check `lws-proof`: every proof/*.json record is well-formed.
...
Usage:
    python scripts/lws_check_proof.py

Stdlib only. Exits 0 and prints "lws-proof check passed" (or a skipped
...
def _validate_record(path: Path, data: object) -> list[str]:
    ...
    try:
        ...
        # or lws_handoff.py's PROOF_DIR monkeypatched for a test) -- fall
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    if count == 0:
        print("lws-proof check skipped (no proof/*.json records yet)")
    else:
        print(f"lws-proof check passed ({count} record(s))")
    return 0
