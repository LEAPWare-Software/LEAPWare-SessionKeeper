#!/usr/bin/env python3
"""Validate plugins/codex/lwr against the shape this project's Codex plugin uses.

Checks (stdlib only):
  - .codex-plugin/plugin.json exists, is valid JSON, has name/version/
    description, and its "hooks" field points at "./hooks/hooks.json".
  - skills/lwr-config/SKILL.md and skills/lwr-report/SKILL.md exist.
  - hooks/hooks.json exists, is valid JSON, and every command (including
    ...
    absolute path — mirrors scripts/lwr_validate_claude_plugin.py's
  ...
  - bin/lwr_hook.py exists.
  - vendor/lwr_core and vendor/adapters/codex exist (scripts/lwr_build.py has
...
PLUGIN_DIR = REPO_ROOT / "plugins" / "codex" / "lwr"
...
def validate() -> list[str]:
    ...
    for skill in ("lwr-config", "lwr-report"):
    ...
    if not (PLUGIN_DIR / "bin" / "lwr_hook.py").is_file():
        errors.append("missing plugins/codex/lwr/bin/lwr_hook.py")

    vendor_core = PLUGIN_DIR / "vendor" / "lwr_core"
    vendor_adapter = PLUGIN_DIR / "vendor" / "adapters" / "codex"
    if not vendor_core.is_dir():
        errors.append(f"missing {vendor_core} — run scripts/lwr_build.py")
    if not vendor_adapter.is_dir():
        errors.append(f"missing {vendor_adapter} — run scripts/lwr_build.py")
    ...
    if isinstance(marketplace, dict):
        ...
        if "./plugins/codex/lwr" not in paths:
            errors.append(
