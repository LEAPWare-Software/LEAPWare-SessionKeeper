...
For each of `plugins/claude/lwr/hooks/hooks.json` and
`plugins/codex/lwr/hooks/hooks.json`, this takes the literal `command`
...
def _check_one(plugin_dir: Path, root_var: str, fixture: Path, errors: list[str]) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="lwr-portable-") as tmp:
        ...
        env["LWR_LEDGER_PATH"] = str(Path(tmp) / "ledger.jsonl")
