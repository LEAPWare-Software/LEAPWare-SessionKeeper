...
Owner directive 5, as amended by the owner on 2026-09-18: "Codex works only
on the Codex part, Claude only on the Claude part." The lane split is
enforced here. The dual CTO/CIO sign-off on shared paths that the original
wording also required has been removed, so this module no longer looks for
`reviews/<pr>/<agent>-cto.json`.
...
def check_lanes(rev_range: str, pr_number: int) -> list[str]:
    ...
    for sha in commits_in_range(rev_range):
        ...
        for f in commit_files(sha):
            cls = classify_path(f)
            if cls not in ("shared", agent):
                errors.append(
