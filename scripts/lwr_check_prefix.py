#!/usr/bin/env python3
"""CI check `lwr-prefix`: every user-facing entrypoint starts with `lwr`.

Checks (stdlib only):
  - Every `skills/<name>/SKILL.md` directory name under `plugins/**` starts
    with `lwr-`, and its front-matter `name:` field matches the directory.
  - Every `command`, `bin/`, or `scripts/` script a user or plugin manifest
    can invoke directly — `plugins/*/*/bin/*.py` and `scripts/*.py` — starts
    with `lwr`, except this script's own module (checked by name) and
    non-entrypoint helpers explicitly allow-listed below.
  - Both plugin manifests' `"name"` field is exactly `lwr`.

Exits 0 and prints "lwr-prefix check passed" on success; otherwise prints
...
def _check_skills(errors: list[str]) -> None:
    for skill_md in sorted(REPO_ROOT.glob("plugins/*/*/skills/*/SKILL.md")):
        ...
        if not dir_name.startswith("lwr-"):
            errors.append(f"skill directory does not start with 'lwr-': {skill_md.parent}")
            continue
...
def _check_scripts(errors: list[str]) -> None:
    for py in sorted(REPO_ROOT.glob("scripts/*.py")):
        if py.name in _NON_ENTRYPOINT_EXEMPT:
            continue
        if not py.name.startswith("lwr"):
            errors.append(f"scripts/ entrypoint does not start with 'lwr': {py}")

    for py in sorted(REPO_ROOT.glob("plugins/*/*/bin/*.py")):
        if py.name in _NON_ENTRYPOINT_EXEMPT:
            continue
        if not py.name.startswith("lwr"):
            errors.append(f"plugin bin/ entrypoint does not start with 'lwr': {py}")
...
def _check_manifests(errors: list[str]) -> None:
    for manifest_path in sorted(REPO_ROOT.glob("plugins/*/*/.claude-plugin/plugin.json")) + sorted(
        ...
        try:
            ...
            continue
        if data.get("name") != "lwr":
            errors.append(f"{manifest_path}: plugin id is '{data.get('name')}', want 'lwr'")
    ...
    for marketplace_path in (
        ...
        for entry in data.get("plugins", []):
            if isinstance(entry, dict) and entry.get("name") != "lwr":
                errors.append(
                    f"{marketplace_path}: plugin entry id is '{entry.get('name')}', want 'lwr'"
                )
