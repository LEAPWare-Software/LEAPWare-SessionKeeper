...
lives in lwr_core or adapters/claude/hook_io.py, both under vendor/ next to
this file (populated by scripts/lwr_build.py). This file only does I/O and
...
from lwr_core.engine import evaluate  # noqa: E402
from lwr_core.ledger import ledger_record  # noqa: E402


def _resolve_policy_path() -> Path:
    """User override policy, if present, else the bundled default.

    Reads `LWR_POLICY_PATH` if set (for tests and advanced setups, since a
    ...
    override = os.environ.get("LWR_POLICY_PATH")
    if override:
...
def _load_policy_dict(path: Path):
    ...
    than raising, per the fail-open contract in lwr_core/config.py.
    """
    try:
...
def _ledger_path() -> Path:
    ...
    override = os.environ.get("LWR_LEDGER_PATH")
    if override:
        return Path(override)
