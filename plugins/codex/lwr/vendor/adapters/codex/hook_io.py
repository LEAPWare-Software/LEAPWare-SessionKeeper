"""Codex adapter: translate Codex's hook JSON <-> lwr_core's neutral shapes.
...
Claude adapter: see `plugins/codex/lwr/bin/lwr_hook.py` for the thin script
that reads stdin / writes stdout / sets the exit code.

`render_report` and the lwr-report skill are kept: a human-readable report
...
from lwr_core.config import Policy, load_policy_dict
...
#: hooks.json command args (e.g. "${PLUGIN_ROOT}/bin/lwr_hook.py"). Per the
...
#: Claude's CLAUDE_PLUGIN_DATA in plugins/claude/lwr/bin/lwr_hook.py).
...
def render_report(decision: Decision) -> str:
    """Human-readable text for the lwr-report skill. Reporting only — never blocks."""
    lines = []
    if decision.permit:
        lines.append("lwr: would ALLOW under current policy.")
    else:
        lines.append(f"lwr: would DENY under current policy — {decision.deny_reason}")
    for warning in decision.warnings:
        lines.append(f"lwr: warning — {warning}")
    if not decision.findings:
        lines.append("lwr: no rule had an opinion on this event.")
    return "\n".join(lines)
