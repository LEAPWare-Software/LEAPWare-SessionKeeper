---
name: lwr-config
description: Read or edit lwr's token-policy configuration for this session or project (Codex side; the same policy file the enforcing PreToolUse hook in hooks/hooks.json reads).
---

# lwr-config (Codex)

Identical policy format and file to the Claude Code plugin: see
`core/policy/schema.json` and `core/policy/default.json` in the lwr
...
3. Remind the user explicitly: a `deny` mode here is enforced immediately
   ...
   Claude Code plugin — see lwr-report to preview a decision without
