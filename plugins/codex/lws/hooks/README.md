...
equivalent of `plugins/claude/lws/hooks/hooks.json`. Both plugins run the
same `lws_core` engine and the same rules; only the wire format differs.
...
- `hooks.json` — one `PreToolUse` hook, matcher `Agent`, running
  `bin/lws_hook.py` (mirrors `plugins/claude/lws/bin/lws_hook.py`) via the
...
- `bin/lws_hook.py` — reads the hook's stdin JSON, evaluates it through
  `lws_core.engine.evaluate`, appends one ledger line under
