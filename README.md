# LEAPWare SessionKeeper

**New session? Read [HANDOFF.md](HANDOFF.md) first.**

SessionKeeper makes an AI coding session's spend rules **mechanical**: hooks that
...
|---|---|---|
| Claude Code | `plugins/claude/lws/` | Registers an enforcing `PreToolUse` hook. |
| Codex CLI | `plugins/codex/lws/` | Registers an enforcing `PreToolUse` hook, same engine as Claude Code. See [docs/install-codex.md](docs/install-codex.md). |
...
One rule ships today, `lws_version` (see
[docs/rules/lws-version.md](docs/rules/lws-version.md)): a safe no-op that
fires on every event, reports this build's `lws_core.__version__`, and
...
enforcement yet: LWS's actual runway rules (wind-down / landing /
...
```
core/lws_core/          pure engine: (Event, Policy) -> Decision. No I/O.
...
plugins/claude/lws/      the installable Claude Code plugin (vendors core+adapter).
plugins/codex/lws/       the installable Codex plugin (vendors core+adapter).
scripts/lws_build.py          copies core/ + the matching adapter into each plugin's vendor/.
```
...
python scripts/lws_build.py                # refresh both plugins' vendor/ trees
python scripts/lws_build.py --check        # fail if vendor/ has drifted from source
python scripts/lws_validate_claude_plugin.py
python scripts/lws_validate_codex_plugin.py
```
