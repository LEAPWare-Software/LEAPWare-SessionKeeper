"""Tests for scripts/lwr_check_proof.py against tests/fixtures/proof/.
...
import lwr_check_proof  # noqa: E402
...
def test_valid_record_has_no_errors():
    errors = lwr_check_proof._validate_record(FIXTURES / "valid.json", _load("valid.json"))
    assert errors == []


def test_self_certified_record_is_rejected():
    errors = lwr_check_proof._validate_record(FIXTURES / "self_certified.json", _load("self_certified.json"))
    assert any("self-certified" in e for e in errors)


def test_exit_mismatch_record_is_rejected():
    errors = lwr_check_proof._validate_record(FIXTURES / "exit_mismatch.json", _load("exit_mismatch.json"))
    assert any("expect_exit" in e for e in errors)


def test_missing_field_record_is_rejected():
    errors = lwr_check_proof._validate_record(FIXTURES / "missing_field.json", _load("missing_field.json"))
...
def test_real_proof_directory_validates_clean():
    ...
    errors, count = lwr_check_proof.validate_all()
    assert errors == []
    assert count >= 0
