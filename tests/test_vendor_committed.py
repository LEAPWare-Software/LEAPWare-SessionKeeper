...
afterward (scripts/lws_build.py never executes on the install path), so
plugins/*/lws/vendor/ must ship as real, tracked files that are byte-identical
to what scripts/lws_build.py would write today. This test would have caught
#LWS-D0's first CI break (vendor/ gitignored, no tree for CI to check against)
...
vendor/ dir equals the *set* scripts/lws_build.py would produce, one for one.
"""
...
import lws_build  # noqa: E402

VENDOR_DIRS = [
    "plugins/claude/lws/vendor",
    "plugins/codex/lws/vendor",
]
...
def test_vendor_tracked_set_and_content_exactly_matches_build():
    ...
    byte in it, must equal exactly what scripts/lws_build.py would produce
    ...
    """
    with tempfile.TemporaryDirectory(prefix="lws-build-test-") as tmp:
        tmp_root = Path(tmp)
        for host, vendor_dir in lws_build.TARGETS:
            staging = lws_build._build_one(host, vendor_dir, tmp_root)
            ...
            assert tracked_rel == built_files, (
                ...
                f"scripts/lws_build.py produces.\n"
            ...
            assert not mismatched, (
                ...
                f"scripts/lws_build.py's output: {mismatched} "
                f"(run scripts/lws_build.py and commit the result)"
            )
