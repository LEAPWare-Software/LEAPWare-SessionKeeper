"""CTO decision, 2026-09-17: LWS must ship a safe minimal hook, not LWH's
enforcing budget_line rule -- installed next to LWH it would double-enforce,
and it isn't a runway rule. `lws_version` replaces it: allow every event,
...
(`plugins/claude/lws/bin/lws_hook.py`, `plugins/codex/lws/bin/lws_hook.py`)
as subprocesses with NO `LWS_POLICY_PATH` override -- so each hook falls
...
Mutation: change `lws_version.evaluate` to honor `config.mode` (i.e. return
a DENY-mode Finding when configured to deny) and re-run
`python scripts/lws_build.py` to sync the vendor trees --
...
# lws_version fires on every event regardless of prompt content)
HOOK_TARGETS = [
    (
...
def test_both_hooks_allow_under_the_shipped_default_policy(tmp_path):
    for plugin_dir, fixture in HOOK_TARGETS:
        ...
        env = {
            # Deliberately NO LWS_POLICY_PATH: the hook must fall back to
        ...
        assert "lws" in payload["hookSpecificOutput"]["permissionDecisionReason"]
