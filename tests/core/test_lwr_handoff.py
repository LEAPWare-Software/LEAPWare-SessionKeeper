"""Unit tests for scripts/lwr_handoff.py. No network; git/gh calls are
...
SCRIPT_PATH = REPO_ROOT / "scripts" / "lwr_handoff.py"

_spec = importlib.util.spec_from_file_location("lwr_handoff", SCRIPT_PATH)
lwr_handoff = importlib.util.module_from_spec(_spec)
sys.modules["lwr_handoff"] = lwr_handoff
_spec.loader.exec_module(lwr_handoff)


def _valid_text(begin: str = lwr_handoff.BEGIN_MARKER, end: str = lwr_handoff.END_MARKER) -> str:
    return (
        "# HANDOFF\n\n"
        "## Start of session\n- [ ] read this\n\n"
        "## In flight\n1. do the thing\n\n"
...
def test_cmd_check_missing_file(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr(lwr_handoff, "HANDOFF_PATH", tmp_path / "HANDOFF.md")
    assert lwr_handoff.cmd_check() == 1
...
def test_cmd_check_valid_file(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(lwr_handoff, "HANDOFF_PATH", path)
    assert lwr_handoff.cmd_check() == 0
...
def test_cmd_write_regenerates_only_the_block(tmp_path, monkeypatch):
    ...
    path.write_text(_valid_text(), encoding="utf-8")
    monkeypatch.setattr(lwr_handoff, "HANDOFF_PATH", path)
    monkeypatch.setattr(lwr_handoff, "_run_git", lambda args: "cafef00d")
    monkeypatch.setattr(lwr_handoff, "_run_gh", lambda args: "")

    before = path.read_text(encoding="utf-8")
    prose_before = before.split(lwr_handoff.BEGIN_MARKER)[0]

    assert lwr_handoff.cmd_write() == 0

    after = path.read_text(encoding="utf-8")
    prose_after = after.split(lwr_handoff.BEGIN_MARKER)[0]
    assert prose_before == prose_after
    assert "cafef00d" in after
    assert after.count(lwr_handoff.BEGIN_MARKER) == 1
    assert after.count(lwr_handoff.END_MARKER) == 1


def test_cmd_write_requires_existing_markers(tmp_path, monkeypatch, capsys):
    path = tmp_path / "HANDOFF.md"
    path.write_text(_valid_text().replace(lwr_handoff.BEGIN_MARKER, ""), encoding="utf-8")
    monkeypatch.setattr(lwr_handoff, "HANDOFF_PATH", path)
    assert lwr_handoff.cmd_write() == 1
...
def test_cmd_write_records_cli_and_session(tmp_path, monkeypatch):
    ...
    path.write_text(_valid_text(), encoding="utf-8")
    monkeypatch.setattr(lwr_handoff, "HANDOFF_PATH", path)
    monkeypatch.setattr(lwr_handoff, "PROOF_DIR", tmp_path / "no-such-proof-dir")
    monkeypatch.setattr(lwr_handoff, "_run_git", lambda args: "cafef00d")
    monkeypatch.setattr(lwr_handoff, "_run_gh", lambda args: "")

    assert lwr_handoff.cmd_write(cli="claude", session="sess-123") == 0
...
def test_cmd_write_lists_deliverable_proof_state(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(lwr_handoff, "HANDOFF_PATH", path)
    monkeypatch.setattr(lwr_handoff, "PROOF_DIR", proof_dir)
    monkeypatch.setattr(lwr_handoff, "_run_git", lambda args: "cafef00d")
    monkeypatch.setattr(lwr_handoff, "_run_gh", lambda args: "")
...
def test_real_handoff_md_passes_check():
    ...
    assert lwr_handoff._validate(text) == []
