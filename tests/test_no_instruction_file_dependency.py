...
1. A static scan (`scripts/lws_check_no_instruction_dep.py`, also run in CI
...
lws_check_no_instruction_dep = importlib.import_module("lws_check_no_instruction_dep")
...
def test_scan_finds_no_reference_in_this_checkout():
    assert lws_check_no_instruction_dep.find_violations() == []
...
def test_scan_catches_a_planted_reference(tmp_path):
    ...
    original_dirs = lws_check_no_instruction_dep.SCAN_DIRS
    original_root = lws_check_no_instruction_dep.REPO_ROOT
    try:
        lws_check_no_instruction_dep.REPO_ROOT = tmp_path
        lws_check_no_instruction_dep.SCAN_DIRS = ("plugins",)
        violations = lws_check_no_instruction_dep.find_violations()
    finally:
        lws_check_no_instruction_dep.REPO_ROOT = original_root
        lws_check_no_instruction_dep.SCAN_DIRS = original_dirs
...
def test_both_hooks_run_correctly_with_no_instruction_files_up_the_tree(tmp_path, monkeypatch):
    ...
    for plugin_dir, fixture in HOOK_TARGETS:
        hook_script = plugin_dir / "bin" / "lws_hook.py"
        ...
        env = {
            "LWS_POLICY_PATH": str(policy_path),
            "LWS_LEDGER_PATH": str(ledger_path),
