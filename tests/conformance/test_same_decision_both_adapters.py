...
This is the test the walking-skeleton pipeline is built to satisfy: sessionkeeper's
...
fields `lws_version` (or any rule in the registry) reads.
"""
...
DENY_POLICY = Policy(rules={"lws_version": RuleConfig(mode=RuleMode.DENY)})  # lws_version ignores this and still allows
...
def test_warn_mode_decision_renders_identically_both_adapters():
    ...
    warn_policy = Policy(rules={"lws_version": RuleConfig(mode=RuleMode.WARN)})
