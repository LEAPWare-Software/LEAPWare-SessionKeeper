"""Claude Code adapter: translate Claude's hook JSON <-> lws_core's neutral shapes.
...
stdout, or call sys.exit — see `plugins/claude/lws/bin/lws_hook.py` for the
...
from lws_core.config import Policy
...
#: hooks.json command args (e.g. "${CLAUDE_PLUGIN_ROOT}/bin/lws_hook.py").
...
def load_policy(raw: Any) -> Policy:
    ...
    Kept here (rather than making bin/lws_hook.py import lws_core.config
    directly) so every Claude-facing translation lives in one file.
    """
    from lws_core.config import load_policy_dict
